...
def colorize_value(
    ...
    if column == "symbol":
        ...
        return (
            f"{GREEN if current_price > purchase_price else RED}{value:>15.8f}{RESET}"
        )
    elif column in ["original_profit_target", "sell_price"]:
        target_price = purchase_price * (1 + profit_target)
        if current_price > target_price:
            return (
                f"{GREEN}{value:>6.3f}{RESET}%"
                if column == "original_profit_target"
        ...
        return (
            f"{GREEN if current_price > purchase_price else RED}{value:>12.2f}€{RESET}"
        )
    return str(value)
...
def print_portfolio(file_path: str) -> None:
    ...
    try:
        ...
        for symbol, data in portfolio["assets"].items():
            ...
            profit_loss_percent = (
                ((data["current_price"] * data["quantity"]) - total_cost)
                / total_cost
                * 100
                if total_cost > 0
                else 0
            ...
            row = {
                ...
                "original_profit_target": data["original_profit_target"]
