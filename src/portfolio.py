...
from .storage import write_to_google_sheets, load_active_assets
...
def sell_asset(symbol, asset, current_price, portfolio, portfolio_lock, finished_trades, reason, price_monitor_manager):
    """
    ...
    try:
        ...
        with portfolio_lock:
            ...
            asset['take_action'] = ''
            if current_price <= 0:
                logger.error(f"Invalid sell price {current_price} for {symbol}. Cannot sell.")
                return None
            ...
            finished_trade = {
                'Symbol': symbol,
                'Buy Quantity': f"{asset['quantity']:.10f}",  # Increased precision
                'Buy Price': f"{asset['purchase_price']:.10f}",  # Increased precision
                'Buy Time': asset['purchase_time'].strftime('%Y-%m-%d %H:%M:%S'),
                'Buy Fee': f"{buy_fee:.2f}",
                'Sell Quantity': f"{asset['quantity']:.10f}",
                'Sell Price': f"{current_price:.10f}",  # Increased precision
            ...
            logger.info(f"Sold {asset['quantity']:.10f} {symbol} at {current_price:.8f} EUR for {net_sale_value:.2f} EUR "
...
def sell_most_profitable_asset(portfolio, portfolio_lock, percent_changes, finished_trades):
    """
    ...
    with portfolio_lock:
        ...
        profitable_assets = [
            (symbol, asset) for symbol, asset in portfolio['assets'].items()
            if (current_time - asset['purchase_time']).total_seconds() / 60 >= MIN_HOLDING_MINUTES
        ]
        ...
        for symbol, asset in profitable_assets:
            current_price = percent_changes[percent_changes['symbol'] == symbol]['close_price']
            ...
            unrealized_profit = (current_price - asset['purchase_price']) / asset['purchase_price']
        ...
        return sell_asset(symbol, asset, current_price, portfolio, portfolio_lock, finished_trades,
...
def manage_portfolio(above_threshold_data, percent_changes, price_monitor_manager):
    """
    Manages the portfolio by processing manual sell signals, updating assets, and buying new assets.
    ...
    """
    global last_sheets_write, last_sheets_load
    current_time = datetime.utcnow()
    five_min_ago = current_time - timedelta(minutes=5)
    logger.info(f"Portfolio before update: Cash = {portfolio['cash']:.2f} EUR, Assets = {len(portfolio['assets'])}")
    finished_trades = []
    total_asset_value = 0.0
    skipped_assets = []

    # Load manual sell signals from Google Sheets
    if (time.time() - last_sheets_load) >= 60:
        logger.info("Reloading Active_Assets sheet to check for manual sell signals")
        sheet_assets = load_active_assets(GOOGLE_SHEETS_CREDENTIALS, SPREADSHEET_NAME, ACTIVE_ASSETS_SHEET)
        with portfolio_lock:
            logger.debug("Acquired portfolio_lock for sheet sync")
            for symbol, sheet_info in sheet_assets.items():
                if symbol in portfolio['assets']:
                    portfolio['assets'][symbol]['take_action'] = sheet_info['take_action']
                elif sheet_info['quantity'] > 0:
                    portfolio['assets'][symbol] = sheet_info
                    logger.info(f"Added new asset from sheet: {symbol}")
        last_sheets_load = time.time()

    # Process manual sell signals
    try:
        with portfolio_lock:
            logger.debug("Acquired portfolio_lock for manual sell processing")
            assets_to_sell = []
            for symbol, asset in list(portfolio['assets'].items()):
                if asset.get('take_action', '').strip().lower() == 'sell':
                    current_price = percent_changes[percent_changes['symbol'] == symbol]['close_price']
                    if current_price.empty:
                        logger.warning(f"No current price in percent_changes for {symbol}. Fetching ticker price.")
                        current_price = fetch_ticker_price(symbol)
                        if current_price is None:
                            logger.error(f"Failed to fetch price for {symbol}. Skipping manual sell.")
                            skipped_assets.append(symbol)
                            continue
                        current_price = float(current_price)
                    else:
                        current_price = float(current_price.iloc[0])
                    logger.info(f"Manual sell signal detected for {symbol} at {current_price:.4f} EUR")
                    assets_to_sell.append((symbol, asset, current_price))

        for symbol, asset, current_price in assets_to_sell:
            try:
                finished_trades.append(sell_asset(symbol, asset, current_price, portfolio, portfolio_lock, finished_trades, "Manual sell from Google Sheet", price_monitor_manager))
            except Exception as e:
                logger.error(f"Failed to sell {symbol}: {e}", exc_info=True)
                skipped_assets.append(symbol)
    except Exception as e:
        logger.error(f"Error processing manual sells: {e}", exc_info=True)
    ...
    try:
        with portfolio_lock:
            ...
            if len(portfolio['assets']) >= ASSET_THRESHOLD and above_threshold_data and portfolio['cash'] >= PORTFOLIO_VALUE * ALLOCATION_PER_TRADE:
                profitable_assets = [
                    symbol for symbol, asset in portfolio['assets'].items()
                    if asset['current_price'] > asset['purchase_price'] * 1.01
                    and (datetime.utcnow() - asset['purchase_time']).total_seconds() / 60 >= MIN_HOLDING_MINUTES
                ]
                if profitable_assets:
                    for symbol in profitable_assets:
                        portfolio['assets'][symbol]['profit_target'] = min(portfolio['assets'][symbol]['profit_target'], ADJUSTED_PROFIT_TARGET)
                        logger.info(f"Adjusted profit target for {symbol} to {ADJUSTED_PROFIT_TARGET}")
                else:
                    finished_trades.append(sell_most_profitable_asset(portfolio, portfolio_lock, percent_changes, finished_trades))
            ...
            for symbol in list(portfolio['assets'].keys()):
                if symbol not in low_volatility_assets and symbol not in price_monitor_manager.running:
                    price_monitor_manager.start(symbol, portfolio, portfolio_lock, percent_changes)
                ...
                if current_price.empty:
                    ...
                    current_price = fetch_ticker_price(symbol)
                    if current_price is None:
                        logger.error(f"Failed to fetch price for {symbol}. Using purchase_price for portfolio value.")
                        skipped_assets.append(symbol)
                        continue
                    current_price = float(current_price)
                else:
                ...
                if current_price > 0:
                    ...
                    logger.warning(f"Invalid current_price {current_price} for {symbol}. Using purchase_price.")
                ...
                holding_minutes = (current_time - asset['purchase_time']).total_seconds() / 60
                symbol_candles = percent_changes[percent_changes['symbol'] == symbol].tail(5)
                atr = np.mean(symbol_candles['high'] - symbol_candles['low']) if len(symbol_candles) >= 5 and 'high' in symbol_candles and 'low' in symbol_candles else 0
                trailing_stop = (TRAILING_STOP_FACTOR_EARLY if holding_minutes < 15 else TRAILING_STOP_FACTOR) * atr / purchase_price if atr > 0 else 0.05
                profit_target = max(0.015, min(0.05, 1.2 * atr / purchase_price)) if atr > 0 else asset.get('profit_target', 0.015)
                if len(portfolio['assets']) >= ASSET_THRESHOLD and current_price > purchase_price * 1.01:
                ...
                ema_period = calculate_dynamic_ema_period(
                    holding_minutes, TIME_STOP_MINUTES, len(portfolio['assets']), ASSET_THRESHOLD
                )
                ema_dynamic = calculate_ema(symbol_candles['close'].values, ema_period) if len(symbol_candles) >= ema_period else current_price
                momentum = percent_changes[percent_changes['symbol'] == symbol]['percent_change'].iloc[0] if not percent_changes[percent_changes['symbol'] == symbol].empty else 0
                ...
                if purchase_price > 0:
                    ...
                    logger.error(f"Purchase price for {symbol} is zero, skipping unrealized profit calculation.")
                    skipped_assets.append(symbol)
                    continue
                trailing_loss = (highest_price - current_price) / highest_price if highest_price > purchase_price else 0
                sell_prices = []
                ...
                catastrophic_loss = unrealized_profit <= CAT_LOSS_THRESHOLD and abs(unrealized_profit) > 2 * atr / purchase_price if atr > 0 else False
                time_stop = holding_minutes >= TIME_STOP_MINUTES and unrealized_profit < 0
                multiplied_profit_target = unrealized_profit >= PROFIT_TARGET_MULTIPLIER * profit_target
                regular_sell_signal = (
                    holding_minutes >= MIN_HOLDING_MINUTES and
                    (
                        trailing_loss >= trailing_stop or
                        unrealized_profit >= profit_target or
                        negative_momentum_counts.get(symbol, 0) >= MOMENTUM_CONFIRM_MINUTES
                    )
                )
                sell_signal = multiplied_profit_target or regular_sell_signal or catastrophic_loss or time_stop
                if sell_signal:
                    reason = (
                        'Catastrophic loss' if catastrophic_loss else
                        'Time stop' if time_stop else
                        f"Multiplied profit target ({PROFIT_TARGET_MULTIPLIER}x = {(PROFIT_TARGET_MULTIPLIER * profit_target)*100:.1f}%)" if multiplied_profit_target else
                    ...
                    finished_trades.append(sell_asset(symbol, asset, current_price, portfolio, portfolio_lock, finished_trades, reason))

            # Buy new assets
            for record in above_threshold_data:
                symbol = record['symbol']
                if (symbol not in portfolio['assets'] and
                    portfolio['cash'] >= PORTFOLIO_VALUE * ALLOCATION_PER_TRADE and
                    len(portfolio['assets']) < MAX_ACTIVE_ASSETS and
                    ...
                    trade_count, largest_trade_volume_eur = fetch_trade_details(symbol, five_min_ago, current_time)
                    buy_trade_data = {
                        'Symbol': symbol,
                    ...
                    logger.info(f"Bought {quantity:.4f} {symbol} at {purchase_price:.4f} EUR for {net_allocation:.2f} EUR (after {buy_fee:.2f} fee), "
                                f"Trade Count: {trade_count}, Largest Trade Volume: €{largest_trade_volume_eur:.2f}")
                    price_monitor_manager.start(symbol, portfolio, portfolio_lock, percent_changes)
                    from .notifications import run_async
                    run_async(send_telegram_message(f"Bought {quantity:.4f} {symbol} at {purchase_price:.4f} EUR"))
                elif len(portfolio['assets']) >= MAX_ACTIVE_ASSETS:
                    logger.warning(f"Cannot buy {symbol}: Maximum active assets ({MAX_ACTIVE_ASSETS}) reached.")
                    finished_trades.append(sell_most_profitable_asset(portfolio, portfolio_lock, percent_changes, finished_trades))
                    ...
                    logger.warning(f"Cannot buy {symbol}: Insufficient cash ({portfolio['cash']:.2f} EUR).")

    except Exception as e:
        logger.error(f"Error in portfolio management: {e}", exc_info=True)

    try:
        # Remove save_portfolio() call (handled in state.py or main.py)
        if time.time() - last_sheets_write >= SHEETS_WRITE_INTERVAL or finished_trades:
            if portfolio['assets']:
                write_to_google_sheets(portfolio['assets'], GOOGLE_SHEETS_CREDENTIALS, SPREADSHEET_NAME, ACTIVE_ASSETS_SHEET, is_active_assets=True)
            else:
                write_to_google_sheets([], GOOGLE_SHEETS_CREDENTIALS, SPREADSHEET_NAME, ACTIVE_ASSETS_SHEET, is_active_assets=True)
            if finished_trades:
                write_to_google_sheets(finished_trades, GOOGLE_SHEETS_CREDENTIALS, SPREADSHEET_NAME, FINISHED_TRADES_SHEET, is_finished_trades=True)
    ...
    if skipped_assets:
        logger.info(f"Portfolio value may be inaccurate due to missing prices for: {', '.join(skipped_assets)}")
    logger.info(f"Portfolio: Cash: {portfolio['cash']:.2f} EUR, Assets: {len(portfolio['assets'])}, Total Value: {total_portfolio_value:.2f} EUR")