...
from .config import (ACTIVE_ASSETS_SHEET, ADJUSTED_PROFIT_TARGET,
                     ALLOCATION_PER_TRADE, ASSET_THRESHOLD, BUY_FEE,
                     CAT_LOSS_THRESHOLD, FINISHED_TRADES_SHEET,
                     GOOGLE_SHEETS_CREDENTIALS, MAX_ACTIVE_ASSETS,
...
from .storage import load_active_assets, write_to_google_sheets
...
def manage_portfolio(above_threshold_data, percent_changes, price_monitor_manager):
    ...
    if (time.time() - last_sheets_load) >= 60:
        ...
        sheet_assets = load_active_assets(
            GOOGLE_SHEETS_CREDENTIALS, SPREADSHEET_NAME, ACTIVE_ASSETS_SHEET
        )
        with portfolio_lock:
            ...
            for symbol, sheet_info in sheet_assets.items():
                if symbol in portfolio["assets"]:
                    portfolio["assets"][symbol]["take_action"] = sheet_info[
                        "take_action"
                    ]
            ...
            for symbol, asset in list(portfolio["assets"].items()):
                if asset.get("take_action", "").strip().lower() == "sell":
                    ...
                    if current_price.empty:
                        ...
                        if current_price is None:
                            logger.error(
                                f"Failed to fetch price for {symbol}. Skipping manual sell."
                            )
                        ...
                        current_price = float(current_price.iloc[0])
                    logger.info(
                        f"Manual sell signal detected for {symbol} at {current_price:.4f} EUR"
                    )
                ...
                if (
                    ...
                    run_async(
                        send_telegram_message(
                            f"Bought {quantity:.4f} {symbol} at {purchase_price:.4f} EUR"
                        )
                    )
