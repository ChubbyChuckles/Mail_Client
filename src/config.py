...
log_filename = os.path.join(log_dir, f"trading_{SESSION_START_TIME.strftime('%Y%m%d_%H%M%S')}.log")
...
WEIGHT_PER_REQUEST = int(os.getenv('WEIGHT_PER_REQUEST', 1))
...
ALLOCATION_PER_TRADE = float(os.getenv('ALLOCATION_PER_TRADE', 0.05))
BUY_FEE = float(os.getenv('BUY_FEE', 0.0025))
...
if MAX_ACTIVE_ASSETS < 1:
    logger.warning(f"MAX_ACTIVE_ASSETS is {MAX_ACTIVE_ASSETS}, must be >= 1. Setting to 20.")
