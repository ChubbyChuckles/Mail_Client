...
def calculate_ema(prices, period):
    ...
    try:
        ...
        if len(prices_series) < period:
            logger.warning(f"Insufficient data for EMA calculation: {len(prices_series)} prices, need {period}")
            return None
...
def calculate_dynamic_ema_period(holding_minutes, time_stop_minutes, active_assets, asset_threshold):
    """
    ...
    try:
        ...
        logger.debug(f"Calculated dynamic EMA period: {period} (holding: {holding_minutes:.2f} min, "
...
def append_to_buy_trades_csv(trade_data):
    ...
    try:
        ...
        logger.info(f"Appended buy trade for {trade_data['Symbol']} to {BUY_TRADES_CSV}")
    except Exception as e:
