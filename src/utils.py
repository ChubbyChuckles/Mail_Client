...
def calculate_dynamic_ema_period(
    ...
    try:
        ...
        logger.error(
            f"Unexpected error calculating dynamic EMA period: {e}", exc_info=True
        )
        send_alert(
            "Dynamic EMA Period Failure",
            f"Unexpected error calculating dynamic EMA period: {e}",
        )
        return 5
...
def append_to_buy_trades_csv(trade_data):
    ...
    try:
        ...
        if not all(field in trade_data for field in required_fields):
            raise ValueError(
                f"trade_data missing required fields: {set(required_fields) - set(trade_data.keys())}"
            )
        ...
        if file_exists:
            with open(file_path, mode="r", newline="") as f:
                ...
                if set(reader.fieldnames) != set(required_fields):
                    logger.warning(
                        f"Fieldnames mismatch in {file_path}. Existing: {reader.fieldnames}"
                    )
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", delete=False, newline="", suffix=".tmp"
        ) as temp_file:
        ...
        logger.error(
            f"Error appending to {config.config.BUY_TRADES_CSV}: {e}", exc_info=True
        )
        send_alert("Buy Trades CSV Failure", f"Error appending to buy trades CSV: {e}")
    except Exception as e:
        logger.error(
            f"Unexpected error appending to {config.config.BUY_TRADES_CSV}: {e}",
            exc_info=True,
        )
        send_alert(
            "Buy Trades CSV Failure",
            f"Unexpected error appending to buy trades CSV: {e}",
        )
...
def append_to_finished_trades_csv(trade_data):
    ...
    try:
        ...
        move_file_with_retry(temp_file.name, file_path)
        logger.info(
            f"Appended finished trade for {trade_data['Symbol']} to {file_path}"
        )
    except (ValueError, OSError) as e:
        logger.error(
            f"Error appending to {config.config.FINISHED_TRADES_CSV}: {e}",
            exc_info=True,
        )
        send_alert(
            "Finished Trades CSV Failure",
            f"Error appending to finished trades CSV: {e}",
        )
    except Exception as e:
        logger.error(
            f"Unexpected error appending to {config.config.FINISHED_TRADES_CSV}: {e}",
            exc_info=True,
        )
        send_alert(
            "Finished Trades CSV Failure",
            f"Unexpected error appending to finished trades CSV: {e}",
        )
...
def append_to_order_book_metrics_csv(metrics_list):
    ...
    try:
        ...
        for metrics in metrics_list:
            ...
            row = {
                ...
                "Best_Bid": (
                    ...
                    if metrics.get("best_bid") is not None
                    else None
                ),
                "Best_Ask": (
                    f"{metrics['best_ask']:.2f}"
                    if metrics.get("best_ask") is not None
                    else None
                ),
                "Spread": (
                    f"{metrics['spread']:.2f}"
                ...
                "Spread_Percentage": (
                    ...
                    if metrics.get("spread_percentage") is not None
                    else None
                ),
                "Mid_Price": (
                    f"{metrics['mid_price']:.2f}"
                    if metrics.get("mid_price") is not None
                    else None
                ),
                "Buy_Depth": (
                    f"{metrics['buy_depth']:.2f}"
                    if metrics.get("buy_depth") is not None
                    else None
                ),
                "Sell_Depth": (
                    f"{metrics['sell_depth']:.2f}"
                    if metrics.get("sell_depth") is not None
                    else None
                ),
                "Total_Depth": (
                    f"{metrics['total_depth']:.2f}"
                    if metrics.get("total_depth") is not None
                    else None
                ),
                "Bid_Volume": (
                    f"{metrics['bid_volume']:.8f}"
                    if metrics.get("bid_volume") is not None
                    else None
                ),
                "Ask_Volume": (
                    f"{metrics['ask_volume']:.8f}"
                    if metrics.get("ask_volume") is not None
                    else None
                ),
                "Bid_Value": (
                    f"{metrics['bid_value']:.2f}"
                    if metrics.get("bid_value") is not None
                    else None
                ),
                "Ask_Value": (
                    f"{metrics['ask_value']:.2f}"
                    if metrics.get("ask_value") is not None
                    else None
                ),
                "Order_Book_Imbalance": (
                    f"{metrics['order_book_imbalance']:.2f}"
                    if metrics.get("order_book_imbalance") is not None
                    else None
                ),
                ...
                "Avg_Bid_Price": (
                    ...
                    if metrics.get("avg_bid_price") is not None
                    else None
                ),
                "Avg_Ask_Price": (
                    f"{metrics['avg_ask_price']:.2f}"
                    if metrics.get("avg_ask_price") is not None
                    else None
                ),
                "VWAP_Bid": (
                    f"{metrics['vwap_bid']:.2f}"
                    if metrics.get("vwap_bid") is not None
                    else None
                ),
                "VWAP_Ask": (
                    f"{metrics['vwap_ask']:.2f}"
                    if metrics.get("vwap_ask") is not None
                    else None
                ),
                "Slippage_Buy": (
                    f"{metrics['slippage_buy']:.2f}"
                    if metrics.get("slippage_buy") is not None
                    else None
                ),
                "Predicted_Price_Buy": (
                    f"{metrics['predicted_price_buy']:.2f}"
                    if metrics.get("predicted_price_buy") is not None
                    else None
                ),
                "Slippage_Sell": (
                    f"{metrics['slippage_sell']:.2f}"
                    if metrics.get("slippage_sell") is not None
                    else None
                ),
                "Predicted_Price_Sell": (
                    f"{metrics['predicted_price_sell']:.2f}"
                    if metrics.get("predicted_price_sell") is not None
                    else None
                ),
        ...
        logger.error(
            f"Error appending to {config.config.ORDER_BOOK_METRICS_CSV}: {e}",
            exc_info=True,
        )
        send_alert(
            "Order Book Metrics CSV Failure",
            f"Error appending to order book metrics CSV: {e}",
        )
    except Exception as e:
        logger.error(
            f"Unexpected error appending to {config.config.ORDER_BOOK_METRICS_CSV}: {e}",
            exc_info=True,
        )
        send_alert(
            "Order Book Metrics CSV Failure",
            f"Unexpected error appending to order book metrics CSV: {e}",
        )
