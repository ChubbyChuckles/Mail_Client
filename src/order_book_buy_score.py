...
def calculate_order_book_imbalance(order_book, mid_price):
    ...
    try:
        ...
        if bid_liquidity == 0.0 and ask_liquidity == 0.0:
            logger.warning("No bids or asks within ±1% of mid-price. Imbalance ratio set to None.")
            return None, bid_liquidity, ask_liquidity
        if ask_liquidity == 0.0:
            logger.warning("No asks within ±1% of mid-price. Imbalance ratio set to None.")
            return None, bid_liquidity, ask_liquidity
        if bid_liquidity == 0.0:
            logger.warning("No bids within ±1% of mid-price. Imbalance ratio set to 0.0.")
    ...
    try:
        ...
        for price, amount in order_book['asks']:
            ...
            if liquidity_ratio < liquidity_threshold:
                ask_cascade_points.append((price, amount, f"Low liquidity ({liquidity_ratio:.2%})"))
...
def evaluate_buy_decision(order_book, mid_price):
    ...
    try:
        ...
        imbalance_score = 0.0 if imbalance_ratio is None else min(1.0, imbalance_ratio / 1.5)  # >1.2 is strong buying pressure
        logger.info(f"Imbalance ratio: {imbalance_ratio if imbalance_ratio is not None else 'None'} "
        ...
        cascade_score = min(1.0, cascade_count / 20)  # >10 vulnerable levels is high risk/reward
        ...
        logger.info(f"Spread: {spread*100:.4f}%{' (None)' if spread is None else ''}, Score: {spread_score:.2f}")
...
def analyze_buy_decision(order_book):
...
#     logger.info(f"Starting buy decision analysis for {symbol} in {json_file} with cascade points in +5% range")
