...
from .config import PRICE_INCREASE_THRESHOLD, MIN_VOLUME_EUR, logger
from .state import low_volatility_assets, portfolio_lock, portfolio
...
def verify_and_analyze_data(df, price_monitor_manager):
    ...
    percent_changes = pd.DataFrame({
        'symbol': latest.index,
        'percent_change': ((latest['close'] - symbols['open']) / symbols['open'] * 100).where(symbols['open'] > 0),
        'open_price': symbols['open'],
        'close_price': latest['close'],
        'latest_timestamp': latest['timestamp'],
        'volume_eur': (latest['volume'] * latest['close']).where(latest['volume'] > 0)
    }).dropna()
    ...
    if not above_threshold.empty:
        ...
        logger.info(f"No coins with price increase >= {PRICE_INCREASE_THRESHOLD}% and volume >= €{MIN_VOLUME_EUR}")

    below_threshold = percent_changes[percent_changes['percent_change'] < PRICE_INCREASE_THRESHOLD]
    if not below_threshold.empty:
        top_5_below = below_threshold.sort_values(by='percent_change', ascending=False).head(5)
    ...
    with portfolio_lock:
        for symbol in list(low_volatility_assets):
            if symbol in portfolio['assets'] and symbol in percent_changes.index:
                recent_change = percent_changes.loc[percent_changes['symbol'] == symbol, 'percent_change'].iloc[0]
                if abs(recent_change) >= PRICE_INCREASE_THRESHOLD / 2:
                    logger.info(f"{symbol} regained volatility (change: {recent_change:.2f}%). Resuming monitoring.")
