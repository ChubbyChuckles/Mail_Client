    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            response = client.request(
                "pool.ntp.org", version=3, timeout=API_TIMEOUT_SECONDS
            )
            time_diff = abs(response.tx_time - time.time())
            logging.info(f"Time sync check: Difference {time_diff:.2f} seconds")
            if time_diff > 2:
                logging.warning(
                    "System clock is off by more than 2 seconds. Sync recommended."
                )
            return
        except Exception as e:
            logging.error(
                f"NTP sync failed (attempt {attempt + 1}/{MAX_RETRIES}): {str(e)}"
            )
...
def get_market_info(bitvavo, symbol):
    ...
    try:
        ...
        logging.error(
            f"Failed to fetch market info: {str(e)}. Using default precision."
        )
        return {"amount_precision": 8, "price_precision": 5, "min_amount": 0.0}


def save_state(
    initial_price, grid_range, buy_prices, sell_prices, buy_sizes, sell_sizes
):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            if bitvavo.getRemainingLimit() < RATE_LIMIT_THRESHOLD:
                logging.warning(
                    f"API rate limit low ({bitvavo.getRemainingLimit()}). Pausing for 60 seconds."
                )
                sleep(60)
            ...
            bids = [
                ...
                for price, amount in order_book.get("bids", [])
            ]
            asks = [
                [float(price), float(amount)]
            ...
            if not bids or not asks:
                logging.warning(
                    "Empty order book received. Using single bid/ask level."
                )
                bids = [[bid, 0]]
                asks = [[ask, 0]]
            ...
            logging.error(
                f"Failed to fetch market data or order book (attempt {attempt + 1}/{MAX_RETRIES}): {str(e)}"
            )
            if attempt < MAX_RETRIES - 1:
                sleep(RETRY_DELAY_SECONDS * (2**attempt))
    logging.warning(
        "Failed to fetch market data after max retries. Using last known price or exiting."
    )
...
def get_balance(bitvavo, symbol):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            logging.error(
                f"Failed to fetch balance (attempt {attempt + 1}/{MAX_RETRIES}): {str(e)}"
            )
            if attempt < MAX_RETRIES - 1:
                sleep(RETRY_DELAY_SECONDS * (2**attempt))
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if len(candles) < periods + 1:
                raise ValueError(
                    f"Not enough candles ({len(candles)}) for ATR period ({periods})."
                )
            ...
            if len(true_ranges) < periods:
                raise ValueError(
                    f"Not enough True Ranges ({len(true_ranges)}) for ATR period ({periods})."
                )
            atr = sum(true_ranges[-periods:]) / periods
            return atr
        except Exception as e:
            logging.error(
                f"Failed to calculate ATR (attempt {attempt + 1}/{MAX_RETRIES}): {str(e)}"
            )
            if attempt < MAX_RETRIES - 1:
                sleep(RETRY_DELAY_SECONDS * (2**attempt))
...
def calculate_grid_levels(
    ...
    try:
        ...
        buy_prices = (
            [round(lower_bound + i * buy_step, 2) for i in range(num_buy_orders)]
            if num_buy_orders > 0
            else []
        )
        sell_prices = (
            [
                round(current_price + i * sell_step, 2)
...
def adjust_grid_for_liquidity(
    ...
    try:
        ...
        for price in buy_prices:
            closest_bid = min(
                bids, key=lambda x: abs(x[0] - price), default=[price, 0]
            )[0]
        ...
        for price in sell_prices:
            closest_ask = min(
                asks, key=lambda x: abs(x[0] - price), default=[price, 0]
            )[0]
        ...
        total_bid_volume = sum(
            ...
            if current_price - grid_range <= price <= current_price
        )
        total_ask_volume = sum(
            amount * price
            for price, amount in asks
            if current_price <= price <= current_price + grid_range
        )
        if total_bid_volume + total_ask_volume < MIN_LIQUIDITY_QUOTE:
            logging.warning(
                f"Insufficient liquidity ({total_bid_volume + total_ask_volume:.2f} EUR) in grid range. Using unadjusted prices."
            )
            return buy_prices, sell_prices
        return adjusted_buy_prices, adjusted_sell_prices
    except Exception as e:
        logging.error(
            f"Error adjusting grid for liquidity: {str(e)}. Using unadjusted prices."
        )
        return buy_prices, sell_prices
...
def calculate_order_sizes(
    ...
    try:
        ...
        if base_size < MIN_ORDER_SIZE_QUOTE:
            ...
            raise ValueError(
                f"Order size {base_size:.2f} EUR is below minimum {MIN_ORDER_SIZE_QUOTE} EUR. Increase TOTAL_CAPITAL_QUOTE to at least {min_capital} EUR or reduce orders."
            )
        ...
        for i in range(num_buy_orders):
            ...
            closest_bid = min(
                bids,
                key=lambda x: abs(x[0] - buy_prices[i]),
                default=[buy_prices[i], 0],
            )[1]
            liquidity_factor = (
                min(closest_bid * buy_prices[i] / base_size, 1.0)
                if closest_bid > 0
                else 1.0
            )
        ...
        for i in range(num_sell_orders):
            ...
            closest_ask = min(
                asks,
                key=lambda x: abs(x[0] - sell_prices[i]),
                default=[sell_prices[i], 0],
            )[1]
            liquidity_factor = (
                min(closest_ask * sell_prices[i] / base_size, 1.0)
                if closest_ask > 0
                else 1.0
            )
        ...
        required_base = sum(
            size / price for price, size in zip(sell_prices, sell_sizes) if size > 0
        )

        # Scale buy order sizes if quote balance is insufficient
        if required_quote > quote_balance:
            logging.warning(
                f"Insufficient quote balance ({quote_balance:.2f} {quote_currency}) for buy orders. Required: {required_quote:.2f}. Scaling buy order sizes."
            )
            scale_factor = quote_balance / required_quote if required_quote > 0 else 0
            buy_sizes = [
                max(size * scale_factor, MIN_ORDER_SIZE_QUOTE) for size in buy_sizes
            ]
            total_buy_size = sum(buy_sizes)
            logging.info(
                f"Scaled buy order sizes: {[round(s, 2) for s in buy_sizes]} {quote_currency}"
            )

        # Scale sell order sizes if base balance is insufficient
        if required_base > base_balance:
            logging.warning(
                f"Insufficient base balance ({base_balance:.8f} {base_currency}) for sell orders. Required: {required_base:.8f}. Scaling sell order sizes."
            )
            scale_factor = base_balance / required_base if required_base > 0 else 0
            sell_sizes = [
                max(size * scale_factor, MIN_ORDER_SIZE_QUOTE) for size in sell_sizes
            ]
            total_sell_size = sum(sell_sizes)
            logging.info(
                f"Scaled sell order sizes: {[round(s, 2) for s in sell_sizes]} {quote_currency}"
            )
        ...
        if total_size > total_capital_quote:
            ...
            logging.info(
                f"Adjusted order sizes to fit total capital: Buy sizes {[round(s, 2) for s in buy_sizes]}, Sell sizes {[round(s, 2) for s in sell_sizes]}"
            )
...
def recover_grid_state(
    bitvavo, symbol, total_capital_quote, num_buy_orders, num_sell_orders
):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if state:
                ...
                buy_prices, sell_prices, _, _ = calculate_grid_levels(
                    initial_price, atr, ATR_MULTIPLIER, num_buy_orders, num_sell_orders
                )
            ...
            order_ids = [
                order["orderId"] for order in orders if order.get("status") == "new"
            ]
            ...
            logging.error(
                f"Failed to recover grid state (attempt {attempt + 1}/{MAX_RETRIES}): {str(e)}"
            )
            if attempt < MAX_RETRIES - 1:
                sleep(RETRY_DELAY_SECONDS * (2**attempt))
        ...
        try:
            ...
            for price, size in zip(buy_prices, buy_sizes):
                ...
                if amount < min_amount:
                    logging.warning(
                        f"Buy order amount {amount} {base_currency} at price {price} below minimum {min_amount}. Skipping."
                    )
                    continue
                if size < MIN_ORDER_SIZE_QUOTE:
                    logging.warning(
                        f"Buy order size {size:.2f} EUR at price {price} below minimum {MIN_ORDER_SIZE_QUOTE}. Skipping."
                    )
                    continue
                maker_price = round_to_precision(
                    min(price, bid - 0.01), price_precision
                )
                ...
                if "orderId" in response:
                    ...
                    logging.info(
                        f"Placed buy order: {amount} {base_currency} @ {maker_price}, Est. fee: {fee:.2f} EUR"
                    )
                else:
            ...
            if base_balance >= required_base:
                for price, size in zip(sell_prices, sell_sizes):
                    ...
                    if amount < min_amount:
                        logging.warning(
                            f"Sell order amount {amount} {base_currency} at price {price} below minimum {min_amount}. Skipping."
                        )
                        continue
                    if size < MIN_ORDER_SIZE_QUOTE:
                        logging.warning(
                            f"Sell order size {size:.2f} EUR at price {price} below minimum {MIN_ORDER_SIZE_QUOTE}. Skipping."
                        )
                        continue
                    maker_price = round_to_precision(
                        max(price, ask + 0.01), price_precision
                    )
                    response = bitvavo.placeOrder(
                        market=symbol,
                        side="sell",
                    ...
                    if "orderId" in response:
                        ...
                        fee = size * MAKER_FEE_RATE
                        logging.info(
                            f"Placed sell order: {amount} {base_currency} @ {maker_price}, Est. fee: {fee:.2f} EUR"
                        )
                    else:
                        logging.error(f"Failed to place sell order: {response}")
            else:
                logging.warning(
                    f"Insufficient {base_currency} ({base_balance:.8f}) for full sell orders. Adjusted sizes used."
                )

            return order_ids
        except Exception as e:
            logging.error(
                f"Error placing orders (attempt {attempt + 1}/{MAX_RETRIES}): {str(e)}"
            )
            if attempt < MAX_RETRIES - 1:
                sleep(RETRY_DELAY_SECONDS * (2**attempt))
    ...
    for order_id in order_ids:
        for attempt in range(MAX_RETRIES):
            try:
                ...
                logging.error(
                    f"Failed to cancel order {order_id} (attempt {attempt + 1}/{MAX_RETRIES}): {str(e)}"
                )
                if attempt < MAX_RETRIES - 1:
                    sleep(RETRY_DELAY_SECONDS * (2**attempt))
...
def calculate_profit_loss(
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            all_filled_orders = try_manual_request(
                api_key, api_secret, symbol, order_ids
            )
            ...
            net_unrealized_loss_percent = (
                abs(unrealized_loss) / total_invested if total_invested > 0 else 0
            )
            total_pnl = realized_profit - unrealized_loss - total_fees
            drawdown_percent = (
                -total_pnl / initial_capital if initial_capital > 0 else 0
            )

            logging.info(
                f"Realized profit: {realized_profit:.2f} EUR, Unrealized loss: {unrealized_loss:.2f} EUR, Total fees: {total_fees:.2f} EUR, Drawdown: {drawdown_percent:.2%}"
            )
            return net_unrealized_loss_percent, total_pnl, drawdown_percent, total_fees

        except Exception as e:
            logging.error(
                f"Error calculating profit/loss (attempt {attempt + 1}/{MAX_RETRIES}): {str(e)}"
            )
            if attempt < MAX_RETRIES - 1:
                sleep(RETRY_DELAY_SECONDS * (2**attempt))
    ...
    while True:
        try:
            ...
            if isinstance(orders, dict) and "errorCode" in orders:
                raise Exception(
                    f"Manual API error: {orders.get('errorCode', 'Unknown')}: {orders.get('error', 'No details provided')}"
                )
            ...
            filled_orders = [
                ...
                and (not order_ids or order["orderId"] in order_ids)
            ]
            all_filled_orders.extend(filled_orders)

            if len(orders) < limit:
                break  # No more orders to fetch
            order_id_from = orders[-1].get(
                "orderId"
            )  # Use last order ID for next batch
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            active_order_prices = [
                float(order["price"])
                for order in orders
                if order["orderId"] in order_ids and order["status"] == "new"
            ]
            ...
            for price, size in zip(buy_prices, buy_sizes):
                if not any(abs(p - price) < 0.01 for p in active_order_prices):
                    ...
                    if amount < min_amount:
                        logging.warning(
                            f"Buy order amount {amount} {symbol.split('-')[0]} at price {price} below minimum {min_amount}. Skipping."
                        )
                        continue
                    if size < MIN_ORDER_SIZE_QUOTE:
                        logging.warning(
                    ...
                    if "orderId" in response:
                        ...
                        logging.info(
                            f"Rebalanced buy order: {amount} {symbol.split('-')[0]} @ {maker_price}"
                        )
            ...
            if base_balance >= required_base:
                for price, size in zip(sell_prices, sell_sizes):
                    if not any(abs(p - price) < 0.01 for p in active_order_prices):
                        ...
                        if amount < min_amount:
                            logging.warning(
                                f"Sell order amount {amount} {symbol.split('-')[0]} at price {price} below minimum {min_amount}. Skipping."
                            )
                            continue
                        if size < MIN_ORDER_SIZE_QUOTE:
                            logging.warning(
                        ...
                        if "orderId" in response:
                            new_order_ids.append(response["orderId"])
                            logging.info(
                                f"Rebalanced sell order: {amount} {symbol.split('-')[0]} @ {maker_price}"
                            )
            else:
                logging.warning(
                    f"Insufficient {symbol.split('-')[0]} ({base_balance:.8f}) for sell order rebalancing."
                )

            return new_order_ids
        except Exception as e:
            logging.error(
                f"Error rebalancing grid (attempt {attempt + 1}/{MAX_RETRIES}): {str(e)}"
            )
            if attempt < MAX_RETRIES - 1:
                sleep(RETRY_DELAY_SECONDS * (2**attempt))
...
def grid_trading(symbol, total_capital_quote, num_buy_orders, num_sell_orders):
    ...
    try:
        ...
        recovered = recover_grid_state(
            bitvavo, symbol, total_capital_quote, num_buy_orders, num_sell_orders
        )
        if recovered[0] is not None:
            order_ids, initial_price, buy_prices, sell_prices, buy_sizes, sell_sizes = (
                recovered
            )
            logging.info("Resuming grid from recovered state.")
        else:
            # Get initial data for fresh start
            current_price, bid, ask, bids, asks = get_market_price_and_book(
                bitvavo, symbol
            )
            base_balance, quote_balance = get_balance(bitvavo, symbol)
            atr = calculate_atr(bitvavo, symbol, ATR_PERIODS)
            grid_range = atr * ATR_MULTIPLIER if atr > 0 else current_price * 0.05
            logging.info(
                f"Current price: {current_price}, Bid: {bid}, Ask: {ask}, ATR: {atr}, Grid range: ±{grid_range:.2f}"
            )
            ...
            buy_prices = [
                round_to_precision(p, market_info["price_precision"])
                for p in adjusted_buy_prices
            ]
            sell_prices = [
                round_to_precision(p, market_info["price_precision"])
                for p in adjusted_sell_prices
            ]
            ...
            if quote_balance < required_quote:
                logging.warning(
                    f"Insufficient {symbol.split('-')[1]} ({quote_balance:.2f}) for buy orders after scaling. Need {required_quote:.2f}."
                )
                raise ValueError(
                    f"Insufficient {symbol.split('-')[1]} balance after scaling."
                )
        ...
        while True:
            ...
            base_balance, quote_balance = get_balance(bitvavo, symbol)
            logging.info(
                f"Current price: {current_price}, Base balance: {base_balance}, Quote balance: {quote_balance}"
            )
            ...
            unrealized_loss_percent, total_pnl, drawdown_percent, total_fees = (
            ...
            if (
                unrealized_loss_percent > LOSS_THRESHOLD_PERCENT
                or drawdown_percent > MAX_DRAWDOWN_PERCENT
            ):
                logging.warning(
                    f"Stopping: Unrealized loss {unrealized_loss_percent:.2%} or Drawdown {drawdown_percent:.2%} exceeded"
                )
                cancel_all_orders(bitvavo, symbol, order_ids)
                if os.path.exists(STATE_FILE):
                    os.remove(STATE_FILE)
