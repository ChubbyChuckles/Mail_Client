...
from .state import weight_used, last_reset_time, rate_limit_lock
    ...
    with semaphore:
        ...
        try:
            ...
            largest_trade_volume_eur = max(
                (trade['amount'] * trade['price'] for trade in trades),
                default=0.0
            )
            if trade_count == 0:
            ...
            logger.error(f"Error fetching trade details for {symbol}: {e}")
            return 0, 0.0