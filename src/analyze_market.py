...
def analyze_parquet_file(file_path, time_window_minutes=50):
    ...
    try:
        ...
        df_latest.loc[:, "price_change_pct"] = (
            (df_latest["close"] - df_latest["open"]) / df_latest["open"] * 100
        )
        df_latest.loc[:, "volatility"] = (
        ...
        df_agg = (
            ...
            .agg({"price_change_pct": "mean", "volume": "sum", "volatility": "mean"})
            .reset_index()
        )
        ...
        csm = (
            top_performers["price_change_pct"].mean()
            - bottom_performers["price_change_pct"].mean()
        )
...
def visualize_market_trends_by_weekday(folder_path, time_window_minutes=50):
    ...
    agg_results = (
        df_results.groupby(["weekday", "hour"])
        .agg(
            {
                "bullish_metrics": "mean",
                "total_volume": "mean",
                "trade_signal": lambda x: (
                    pd.Series(x).mode()[0] if not x.empty else "Neutral"
                ),
                "n_coins": "mean",
                "n_missing": "mean",
                "mbi": "mean",
    ...
    all_combinations = pd.DataFrame(
        [(w, h) for w in weekdays for h in hours], columns=["weekday", "hour"]
    )
    agg_results = all_combinations.merge(
        agg_results, on=["weekday", "hour"], how="left"
    )
    agg_results["bullish_metrics"] = agg_results["bullish_metrics"].fillna(
        0.5
    )  # Neutral sentiment
    ...
    agg_results["volume_ratio"] = agg_results["volume_ratio"].fillna(
        1.0
    )  # Neutral ratio
    ...
    metrics = [
        ...
        (
            "volume_ratio_capped",
            "Bullish/Bearish Volume Ratio (Capped)",
            "RdYlGn",
            0.0,
            10.0,
            ".2f",
    ...
    for metric, title, cmap, vmin, vmax, fmt in metrics:
        ...
        plt.close()
        print(
            f"Heatmap for {metric} saved as '{os.path.join(plots_dir, f'{metric}_heatmap.png')}'"
        )

    # Plot trading signals
    plt.figure(figsize=(12, 6))
    signal_values = {"Bullish": 1, "Neutral": 0, "Bearish": -1}
    pivot_signals = agg_results.pivot(
        index="weekday", columns="hour", values="trade_signal"
    )
    pivot_signals = pivot_signals.reindex(weekdays)
    pivot_signals_numeric = pivot_signals.replace(signal_values)
    sns.heatmap(
        pivot_signals_numeric,
        cmap="RdYlGn",
        vmin=-1,
        vmax=1,
        annot=pivot_signals,
    ...
    plt.close()
    print(
        f"Trading signals heatmap saved as '{os.path.join(plots_dir, 'trading_signals_heatmap.png')}'"
    )
    ...
    best_times = agg_results[
        (
            (agg_results["bullish_metrics"] >= 0.6)
            | (agg_results["bullish_metrics"] <= 0.4)
        )
