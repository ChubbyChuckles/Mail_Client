...
def calculate_order_book_metrics(market, amount_quote=5.5, price_range_percent=10.0):
    ...
    try:
        ...
        # 1. Best Bid and Ask
        metrics["best_bid"] = (
            float(order_book["bids"][0][0]) if order_book["bids"] else None
        )
        metrics["best_ask"] = (
            float(order_book["asks"][0][0]) if order_book["asks"] else None
        )
        ...
        for side in ["buy", "sell"]:
            ...
            if total_amount < base_amount:
                ...
                slippage_percent = (
                    (predicted_price - expected_price) / expected_price
                ) * 100
        ...
        metrics["total_score"] = buy_metrics["total_score"]
        metrics["recommendation"] = buy_metrics["recommendation"]
        return metrics
