...
def simulate_order_impact(order_book, order_size, side='buy'):
    ...
    try:
        ...
        if remaining_size > 0:
            logger.warning(f"{side.capitalize()} order of {order_size} not fully filled. Remaining: {remaining_size}")
...
def detect_cascade_points(order_book, mid_price, liquidity_threshold=0.1, gap_threshold=0.001):
    ...
    for side in ['bids', 'asks']:
        for price, amount in order_book[side]:
            price = float(price)
            if (side == 'bids' and price >= price_range['bids']) or (side == 'asks' and price <= price_range['asks']):
...
def visualize_order_book(order_book, symbol, mid_price, cascade_points, order_sizes=[0.1, 1, 5]):
    ...
    try:
        ...
        for order_size in order_sizes:
            for side in ['buy', 'sell']:
                final_price, cleared_levels = simulate_order_impact(order_book, order_size, side)
                if final_price:
                    color = 'blue' if side == 'buy' else 'purple'
                    plt.axvline(final_price, color=color, linestyle='--', alpha=0.5,
...
def analyze_order_book(json_file, symbol):
    ...
    try:
        ...
        cascade_points = detect_cascade_points(order_book, mid_price, liquidity_threshold=0.1, gap_threshold=0.001)
        ...
        for side, points in cascade_points.items():
            if points:
                ...
                for price, amount, reason in points:
                    logger.info(f"  Price: {price:.2f}, Amount: {amount:.4f}, Reason: {reason}")
            else:
        ...
        for side in ['buy', 'sell']:
            ...
            for order_size in order_sizes:
                final_price, cleared_levels = simulate_order_impact(order_book, order_size, side)
                if final_price:
                    initial_price = float(order_book['asks'][0][0]) if side == 'buy' else float(order_book['bids'][0][0])
                    price_impact = (final_price - initial_price) / initial_price * 100
                    logger.info(f"  Order size: {order_size}, Final price: {final_price:.2f}, "
