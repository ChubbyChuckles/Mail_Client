...
def calculate_atr(candles, period):
    ...
    if len(candles) < period + 1:
        raise Exception(
            f"Not enough candles ({len(candles)}) for ATR period ({period})."
        )
    ...
    if len(true_ranges) < period:
        raise Exception(
            f"Not enough True Ranges ({len(true_ranges)}) for ATR period ({period})."
        )
...
def fetch_candles_and_calculate_atr():
    try:
        ...
        if CANDLE_LIMIT < ATR_PERIOD:
            raise Exception(
                f"Candle limit ({CANDLE_LIMIT}) must be at least as large as ATR period ({ATR_PERIOD})."
            )
        if INTERVAL not in [
            "1m",
            "5m",
            ...
            raise Exception(
                f"Invalid interval: {INTERVAL}. Supported intervals: 1m, 5m, 15m, 30m, 1h, 4h, 6h, 8h, 12h, 1d"
            )
        ...
        print("=" * 60)
        print(
            f"{'Timestamp':<20} {'Open':<10} {'High':<10} {'Low':<10} {'Close':<10} {'Volume':<10}"
        )
        print("-" * 60)
        for candle in candles:
            ...
            print(
                f"{formatted_time:<20} {float(candle[1]):<10.2f} {float(candle[2]):<10.2f} "
