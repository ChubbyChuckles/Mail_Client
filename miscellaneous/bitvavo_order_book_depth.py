...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("order_book_snapshot.log"), logging.StreamHandler()],
)
...
def wait_for_minute_reset(bitvavo):
    ...
    while True:
        ...
        if remaining >= RATE_LIMIT_WEIGHT:
            logger.info(
                f"Rate limit reset confirmed at {now.strftime('%Y-%m-%d %H:%M:%S')} (UTC). "
...
def fetch_top_coins(bitvavo, limit=TOP_COINS_LIMIT):
    ...
    try:
        ...
        sorted_tickers = sorted(
            eur_tickers, key=lambda x: float(x.get("volume", 0)), reverse=True
        )

        # Extract top coins (market symbols in ccxt format, e.g., 'BTC/EUR')
        top_coins = [
            ticker["market"].replace("-", "/") for ticker in sorted_tickers[:limit]
        ]
...
def fetch_order_book_snapshot(symbol, depth=MAX_DEPTH, per_request_delay=0):
    ...
    try:
        bitvavo = ccxt.bitvavo(
            {"apiKey": API_KEY, "secret": API_SECRET, "enableRateLimit": False}
        )
        ...
        if "bids" not in order_book or "asks" not in order_book:
            logger.error(
                f"Process {os.getpid()}: Invalid order book response for {symbol}: {order_book}"
            )
        ...
        logger.info(
            f"Process {os.getpid()}: Fetched order book for {symbol} with depth={depth}: "
        ...
        logger.error(
            f"Process {os.getpid()}: Error fetching order book for {symbol}: {e}"
        )
...
def save_combined_snapshots(snapshots, filename):
    ...
    try:
        ...
        logger.info(
            f"Saved combined snapshots to {filename} in {save_time:.3f} seconds"
        )
        return save_time
    except Exception as e:
...
def main():
    ...
    logger.info(
        f"Starting order book snapshot script at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} (CEST)"
    )

    try:
        # Initialize Bitvavo clients
        bitvavo_api = Bitvavo({"APIKEY": API_KEY, "APISECRET": API_SECRET})
        ...
        logger.info(
            f"Processing {len(eur_pairs)} -EUR pairs: {', '.join(eur_pairs[:5])}..."
        )
        ...
        if expected_weight > RATE_LIMIT_WEIGHT:
            logger.error(
                f"Expected weight ({expected_weight}) exceeds rate limit ({RATE_LIMIT_WEIGHT}). Exiting."
            )
            return
        total_requests = len(eur_pairs)
        pairs_per_process = total_requests // MAX_PROCESSES + (
            1 if total_requests % MAX_PROCESSES else 0
        )
        per_request_delay = (
            (CYCLE_DURATION / MAX_PROCESSES) / pairs_per_process
            if total_requests > 0
            else 0
        )
        logger.info(
            f"Expected weight: {expected_weight}/{RATE_LIMIT_WEIGHT}. "
        ...
        while True:
            ...
            if average_fetch_time > 0:
                per_request_delay = max(
                    0.01,
                    (
                        (CYCLE_DURATION / MAX_PROCESSES)
                        - pairs_per_process * average_fetch_time
                    )
                    / pairs_per_process,
                )
                logger.info(
                    f"Updated per-request delay: {per_request_delay:.3f} seconds based on "
            ...
            logger.info(
                f"Starting data collection at {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} (UTC)"
            )
            ...
            if fetch_times:
                ...
                logger.info(
                    f"Updated average fetch time: {average_fetch_time:.3f} seconds based on {len(fetch_times)} successful fetches"
                )

            logger.info(
                f"Successfully fetched snapshots for {successful_pairs}/{len(eur_pairs)} pairs"
            )

            # Save combined snapshots
            timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
            filename = os.path.join(
                SNAPSHOTS_DIR, f"combined_snapshots_{timestamp}.json"
            )
            ...
            logger.info(
                f"Data collection completed in {cycle_time:.3f} seconds "
            ...
            if "BTC-EUR" in snapshots:
                ...
                if extracted:
                    logger.info(
                        f"Example extraction: BTC-EUR snapshot has {len(extracted['bids'])} bids, "
            ...
            if final_remaining is not None and final_remaining < 100:
                logger.warning(
                    f"Low rate limit ({final_remaining}/1000). Waiting for next minute reset."
                )
    ...
    logger.info(
        f"Script completed at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} (CEST)"
    )
