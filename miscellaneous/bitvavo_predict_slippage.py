...
def predict_slippage(market, amount_quote, side):
    ...
    try:
        ...
        if rate_limit is not None and rate_limit < 10:
            logger.warning(f"Low rate limit remaining: {rate_limit}. Aborting slippage prediction.")
            return {'error': f"Low rate limit remaining: {rate_limit}"}
        
        logger.info(f"Predicting slippage for {side} order of {amount_quote} EUR on {market}")
        ...
        if total_amount < base_amount:
            logger.warning(f"Insufficient liquidity in order book to fill {base_amount:.8f} {market.split('-')[0]}")
