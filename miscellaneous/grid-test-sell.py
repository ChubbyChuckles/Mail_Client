...
def place_limit_sell_order():
    try:
        ...
        if amount_base < min_amount:
            raise Exception(f"Calculated amount {amount_base} {SYMBOL.split('-')[0]} is below minimum amount {min_amount} {SYMBOL.split('-')[0]}")
        ...
        if available_base < amount_base:
            raise Exception(f"Insufficient balance: {available_base} {SYMBOL.split('-')[0]} available, {amount_base} required")

        print(f"Placing limit sell order at {limit_price} EUR for {amount_base} {SYMBOL.split('-')[0]}")
