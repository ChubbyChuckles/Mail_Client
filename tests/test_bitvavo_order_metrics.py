...
def mock_config():
    with patch("src.config.API_KEY", "dummy_key"), patch(
        "src.config.API_SECRET", "dummy_secret"
    ):
        yield
...
def sample_buy_decision():
    return {"total_score": 0.75, "recommendation": "Buy"}


# Test successful order book metrics calculation
def test_calculate_order_book_metrics_success(
    mock_bitvavo, sample_order_book, sample_buy_decision
):
    mock_bitvavo.book.return_value = sample_order_book
    with patch(
        "src.bitvavo_order_metrics.analyze_buy_decision",
        return_value=sample_buy_decision,
    ):
        ...
        assert (
            metrics["order_book_imbalance"] == 0.5
...
def test_empty_order_book(mock_bitvavo):
    mock_bitvavo.book.return_value = {
        "market": "BTC-EUR",
        "nonce": 12345,
        "bids": [],
        "asks": [],
...
def test_zero_bid_volume(mock_bitvavo, sample_order_book):
    ...
    with patch(
        "src.bitvavo_order_metrics.analyze_buy_decision",
        return_value={"total_score": 0.5, "recommendation": "Hold"},
    ):
        metrics = calculate_order_book_metrics(market="BTC-EUR")
...
def test_buy_slippage_multiple_levels(mock_bitvavo, sample_buy_decision):
    ...
    with patch(
        ...
        assert metrics["predicted_price_buy"] == pytest.approx(
            predicted_price, abs=1e-3
        )  # Increased tolerance
...
def test_buy_slippage_single_level(mock_bitvavo, sample_buy_decision):
    ...
    with patch(
        ...
        assert (
            metrics["slippage_buy"] == 0.0
        ), "Slippage should be zero for single level"
...
def test_buy_slippage_insufficient_depth(mock_bitvavo, sample_buy_decision):
    ...
    with patch(
        ...
        assert (
            metrics["slippage_buy"] is None
        ), "Slippage should be None due to insufficient depth"
...
def test_buy_slippage_identical_prices(mock_bitvavo, sample_buy_decision):
    ...
    with patch(
        ...
        assert (
            metrics["slippage_buy"] == 0.0
        ), "Slippage should be zero for identical prices"
        assert metrics["predicted_price_buy"] == predicted_price
...
def test_buy_slippage_large_price_gaps(mock_bitvavo, sample_buy_decision):
    ...
    with patch(
        ...
        assert (
            metrics["slippage_buy"] != 0.0
        ), "Slippage should not be zero with price gaps"
        assert metrics["slippage_buy"] == pytest.approx(slippage, abs=1e-4)
        assert metrics["predicted_price_buy"] == pytest.approx(
            predicted_price, abs=1e-2
        )
...
def test_buy_slippage_small_price_diff(mock_bitvavo, sample_buy_decision):
    ...
    with patch(
        ...
        assert (
            metrics["slippage_buy"] != 0.0
        ), "Slippage should not be zero with small price differences"
        assert metrics["slippage_buy"] == pytest.approx(slippage, abs=1e-6)
        assert metrics["predicted_price_buy"] == pytest.approx(
            predicted_price, abs=1e-6
        )
...
def test_buy_slippage_high_amount(mock_bitvavo, sample_buy_decision):
    ...
    with patch(
        ...
        assert (
            metrics["slippage_buy"] != 0.0
        ), "Slippage should not be zero for high amount"
        assert metrics["slippage_buy"] == pytest.approx(slippage, abs=1e-6)
        assert metrics["predicted_price_buy"] == pytest.approx(
            predicted_price, abs=1e-3
        )
...
def test_insufficient_depth_for_slippage(
    mock_bitvavo, sample_order_book, sample_buy_decision
):
    order_book = sample_order_book.copy()
