...
from src.exchange import (
    handle_ban_error, wait_until_ban_lifted, check_rate_limit,
    fetch_ticker_price, fetch_klines, fetch_trade_details,
    is_banned, ban_expiry_time, weight_used, last_reset_time, RATE_LIMIT_WEIGHT
)
...
def test_handle_ban_error():
    ...
    exception = Exception('403 Forbidden: {"errorCode":105, "error":"The ban expires at 1697059200000"}')
...
def test_wait_until_ban_lifted():
    ...
    with patch("src.exchange.logger") as mock_logger, patch("time.sleep") as mock_sleep:
        ...
        mock_logger.info.assert_any_call(f"Waiting 2.00 seconds until ban lifts at {datetime.utcfromtimestamp(future_time)}")
...
def test_fetch_ticker_price_invalid_price(mock_bitvavo):
    ...
    with patch("src.exchange.check_rate_limit"), patch("src.exchange.semaphore"), patch("src.exchange.logger") as mock_logger:
        price = fetch_ticker_price("BTC/EUR")
        assert price is None

def test_fetch_ticker_price_ban(mock_bitvavo):
    """Test fetch_ticker_price with ban error."""
    mock_bitvavo.fetch_ticker.side_effect = ccxt.PermissionDenied('403 Forbidden: {"errorCode":105, "error":"The ban expires at 1697059200000"}')
    with patch("src.exchange.check_rate_limit"), patch("src.exchange.semaphore"), patch("src.exchange.wait_until_ban_lifted"):
        price = fetch_ticker_price("BTC/EUR")
        assert price is None
...
def test_fetch_ticker_price_fallback(mock_bitvavo):
    ...
    with patch("src.exchange.check_rate_limit"), patch("src.exchange.semaphore"), patch("src.price_monitor.PriceMonitorManager") as mock_pmm:
...
def test_fetch_klines_success(mock_bitvavo):
    ...
    mock_bitvavo.fetch_ohlcv.return_value = [[1697059200000, 50000, 51000, 49000, 50500, 10]]
    with patch("src.exchange.check_rate_limit"), patch("src.exchange.semaphore"):
...
def test_fetch_klines_ban(mock_bitvavo):
    ...
    mock_bitvavo.fetch_ohlcv.side_effect = ccxt.PermissionDenied('403 Forbidden: {"errorCode":105, "error":"The ban expires at 1697059200000"}')
    with patch("src.exchange.check_rate_limit"), patch("src.exchange.semaphore"), patch("src.exchange.wait_until_ban_lifted"):
...
def test_fetch_trade_details_success(mock_bitvavo):
    ...
    with patch("src.exchange.check_rate_limit"), patch("src.exchange.semaphore"):
        trade_count, largest_volume = fetch_trade_details("BTC/EUR", start_time, end_time)
