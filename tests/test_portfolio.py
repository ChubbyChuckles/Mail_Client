...
from src.config import ALLOCATION_PER_TRADE, PORTFOLIO_VALUE, logger
...
def test_manage_portfolio_buy(mock_price_monitor, tmp_path):
    ...
    with portfolio_lock:
        ...
        portfolio["assets"] = {}
    with patch(
        "MAIL_CLIENT_TEST.src.portfolio.fetch_trade_details", return_value=(10, 1000.0)
    ):
    ...
    with portfolio_lock:
        ...
        assert portfolio["cash"] < PORTFOLIO_VALUE
        assert (
            portfolio["assets"]["ETH/EUR"]["quantity"]
            == (PORTFOLIO_VALUE * ALLOCATION_PER_TRADE * (1 - 0.0015)) / 2000.0
        )
